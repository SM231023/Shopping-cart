...
for (let i = 0; i < cart.length; i++) {
  ...
  function dfash(){
    document.getElementsByTagName("fashion").innerHTML=" "; 
  }
...
document.getElementById('Prod').onchange=(ele)=>{
    let prodInfo=ShoppingCart.find((produ)=>produ.id==ele.target.value);
    document.getElementById('checkbox').innerHTML=`
    <div  class="display-product" style="background-color:white;border: solid 2px grey">
    <image src="${prodInfo.image}" width= "200px" height="200px" 
    <div style="text-align:center">${prodInfo.category}</div>
    <div style="text-align:center;border:solid 2px black">${prodInfo.title}</div>
    <div style="text-align:center;border:solid 2px black">${"Price $:"+prodInfo.price}</div>
    </div>
    `
